...
export default function Fibonacci({ gameData, hintData, setUsed }) {
    const [complete, setComplete] = React.useState(gameData.find((elem) => elem.link === hintData.ref).used);
    ...
    return (
        <>
            <p className="md:text-3xl text-center text-green-400 px-4 py-2 italic">{'"' + hintData.msg.join("") + '"'}</p>
            {!complete ? 
                <>
                    <p className="md:text-2xl text-gray-900 px-4 py-2 mt-3 mb-10">{hintData.hint[0]}</p>
                    ...
                    <p className="self-end max-w-xl text-right text-xl text-gray-900 px-4 py-2 mb-10">{hintData.hint[1]}</p>
                </> :
                <>
